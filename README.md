...
Since the site is built with pure HTML, CSS, and Vanilla JavaScript, no build process or package installer is required.
...
2. **Run locally:**
   Open `index.html` in any modern browser, or use a local dev server such as VS Code Live Server / Python HTTP server:
   ```bash
...
- **Vanilla JavaScript (ES6+)** — Intersection Observer API, Mouse Movement Event Listeners, Clipboard API
